...
export const makeGroupsSocket = (config) => {
    ...
    const groupMetadata = async (jid) => {
        ...
        groupMetadataCache.set(jid, { data: meta, ts: Date.now() });
        return meta;
    };
    const groupMetadataCacheClear = (jid) => {
        if (jid) groupMetadataCache.delete(jid);
        else groupMetadataCache.clear();
    };
    const groupFetchAllParticipating = async () => {
        ...
        for (const id of Object.keys(data)) {
            groupMetadataCache.set(id, { data: data[id], ts: Date.now() });
        }
        return data;
    };
