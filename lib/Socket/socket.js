...
const makeSocket = (config) => {
    ...
    const requestPairingCode = async (phoneNumber, customCode) => {
        let pairKey;
        if (customCode) {
            const cleaned = customCode.replace(/[^0-9A-Z]/g, '');
            if (cleaned.length !== 8) {
                throw new Error('Custom pairing code must be exactly 8 characters (digits or uppercase letters only)');
            }
            pairKey = cleaned;
        } else {
            ...
            pairKey = '';
            for (let i = 0; i < 8; i++) {
                pairKey += digits[Math.floor(Math.random() * 10)];
            }
        }
