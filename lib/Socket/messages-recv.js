...
const makeMessagesRecvSocket = (config) => {
    ...
    const msgRetryCache = config.msgRetryCounterCache || new (require('node-cache'))({
        stdTTL: Defaults_1.DEFAULT_CACHE_TTLS.MSG_RETRY,
        maxKeys: 10000,
    });
    const callOfferCache = config.callOfferCache || new (require('node-cache'))({
        stdTTL: Defaults_1.DEFAULT_CACHE_TTLS.CALL_OFFER,
        maxKeys: 10000,
    });
    const placeholderResendCache = config.placeholderResendCache || new (require('node-cache'))({
        stdTTL: Defaults_1.DEFAULT_CACHE_TTLS.MSG_RETRY,
        maxKeys: 10000,
    });
    const groupParticipantJidCache = new (require('node-cache'))({
        stdTTL: 10 * 60,
        maxKeys: 10000,
    });
    const identityDebounceCache = new (require('node-cache'))({
        stdTTL: 5 * 60,
        maxKeys: 10000,
    });
    const groupMetadataWarmupCache = new (require('node-cache'))({
        stdTTL: 60,
        maxKeys: 10000,
    });
