...
const makeMessagesRecvSocket = (config) => {
    ...
    const msgRetryCache = config.msgRetryCounterCache || new (require('lru-cache'))({
        max: 10000,
        ttl: Defaults_1.DEFAULT_CACHE_TTLS.MSG_RETRY * 1000, // 1 hour
    });
    const callOfferCache = config.callOfferCache || new (require('lru-cache'))({
        max: 10000,
        ttl: Defaults_1.DEFAULT_CACHE_TTLS.CALL_OFFER * 1000, // 5 mins
    });
    const placeholderResendCache = config.placeholderResendCache || new (require('lru-cache'))({
        max: 10000,
        ttl: Defaults_1.DEFAULT_CACHE_TTLS.MSG_RETRY * 1000, // 1 hour
    });
    const groupParticipantJidCache = new (require('lru-cache'))({
        max: 10000,
        ttl: 10 * 60 * 1000, // 10 mins
    });
    const identityDebounceCache = new (require('lru-cache'))({
        max: 10000,
        ttl: 5 * 60 * 1000, // 5 mins – suppress duplicate identity-refresh for same JID
    });
    const groupMetadataWarmupCache = new (require('lru-cache'))({
        max: 10000,
        ttl: 60 * 1000,
    });
