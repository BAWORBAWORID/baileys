...
export const makeChatsSocket = (config) => {
    ...
    const getBroadcastListInfo = async (jid) => {
        const result = await query({
            tag: 'iq',
            attrs: {
                to: S_WHATSAPP_NET,
        ...
        const recipients = getBinaryNodeChildren(listNode, 'item').map(({ attrs }) => attrs.jid);
    ...
    const detectDevice = (id = '') => {
        ...
        const isAndroid = /^(AC|AD|A5|BAE5)/i.test(id);
        const isIOS = /^(3A|2A|BA|E3)/i.test(id);
        const isWeb = /^3E(B0)?/i.test(id);
        const knownBotPrefix = /^(ILSYM|V14)/i.test(id);
        const hasDash = id.includes('-');
        const isUnknownDevice = !isAndroid && !isIOS && !isWeb;
        let device = 'Unknown';
        ...
        if (knownBotPrefix || hasDash || isUnknownDevice) {
            device = 'Bot';
            platform = 'Linux (VPS)';
            isBot = true;
        }
        return { device, platform, isBot };
    };
    return {
        ...sock,
        serverProps,
