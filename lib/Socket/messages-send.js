...
export const makeMessagesSocket = (config) => {
    ...
    const createParticipantNodes = async (recipientJids, message, extraAttrs, dsmMessage) => {
        ...
        message = patchMessageRequiresBeforeSending(message);
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, statusJidList }) => {
        ...
        if (normalizedMsg?.interactiveResponseMessage?.nativeFlowResponseMessage) {
            extraAttrs['native_flow_name'] = normalizedMsg.interactiveResponseMessage.nativeFlowResponseMessage?.name || 'menu_options';
        }
        if (participant) {
            if (!isGroup && !isStatus) {
        ...
        await authState.keys.transaction(async () => {
            ...
            // Inject biz nodes for button/interactive messages
            if (!isNewsletter && buttonType && !isStatus) {
                const content = getAdditionalNode(buttonType);
                const filteredNode = getBinaryNodeFilter(additionalNodes ? additionalNodes : []);
                if (filteredNode) {
    ...
    const getMessageType = (message) => {
        ...
        if (normalizedMessage.listMessage || normalizedMessage.buttonsMessage || normalizedMessage.interactiveMessage) {
            return 'text';
        }
