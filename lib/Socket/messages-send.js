...
export const makeMessagesSocket = (config) => {
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, statusJidList }) => {
        ...
        try {
            ...
            if ((coreType === 'interactiveMessage' || coreType === 'buttonsMessage') && !hasTag('biz')) {
                if (coreType === 'interactiveMessage') {
                    const name = innerMsg?.interactiveMessage?.nativeFlowMessage?.buttons?.[0]?.name;
                    if (name === 'payment_key_info' || name === 'payment_info') additionalNodes.push({ tag: 'biz', attrs: {}, content: [{ tag: 'interactive', attrs: { type: 'native_flow', v: '1' }, content: [{ tag: 'native_flow', attrs: { name: 'payment_info' } }] }] });
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            if (typeof content === 'object' &&
            ...
            else if (Array.isArray(content?.album)) {
                const items = content.album;
                let imgCount = 0;
                let vidCount = 0;
                for (const item of items) {
                    if (item.image) imgCount++;
                    else if (item.video) vidCount++;
                }
                const parentMsg = await sock.sendMessage(jid, {
                    album: { expectedImageCount: imgCount, expectedVideoCount: vidCount }
                }, options);
                const sentItems = [];
                for (const item of items) {
                    const childContent = { albumParentKey: parentMsg.key.id };
                    if (item.image) childContent.image = item.image;
                    else if (item.video) childContent.video = item.video;
                    if (item.caption) childContent.caption = item.caption;
                    if (item.mimetype) childContent.mimetype = item.mimetype;
                    if (item.ptv) childContent.ptv = item.ptv;
                    const childMsg = await sock.sendMessage(jid, childContent, options);
                    sentItems.push(childMsg);
                }
                return { parent: parentMsg, children: sentItems };
            }
            else {
                ...
                const mergedAdditionalNodes = [...additionalNodes, ...(options.additionalNodes || [])];
                if (options.biz) {
                    const hasBiz = mergedAdditionalNodes.some(n => n.tag === 'biz');
                    if (!hasBiz) {
                        mergedAdditionalNodes.push({
                            tag: 'biz',
                            attrs: {
                                actual_actors: options.biz.actual_actors || '2',
                                host_storage: options.biz.host_storage || '2',
                                privacy_mode_ts: options.biz.privacy_mode_ts || String(Math.floor(Date.now() / 1000)),
                                ...(options.biz.aw_block_disappearance && { aw_block_disappearance: options.biz.aw_block_disappearance })
                            }
                        });
                    }
                }
                await relayMessage(jid, fullMsg.message, {
                    ...
                    additionalNodes: mergedAdditionalNodes
                });
