...
export const makeMessagesSocket = (config) => {
    ...
    const processJpegThumbnails = async (obj, seen = new WeakSet()) => {
        if (typeof obj !== 'object' || !obj || seen.has(obj)) return;
        seen.add(obj);
        for (const key of Object.keys(obj)) {
            const val = obj[key];
            if (key === 'jpegThumbnail' && typeof val === 'string' && (val.startsWith('http://') || val.startsWith('https://'))) {
                try {
                    const res = await fetch(val);
                    obj[key] = Buffer.from(await res.arrayBuffer());
                } catch {}
            } else if (typeof val === 'object' && val) {
                await processJpegThumbnails(val, seen);
            }
        }
    };
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, statusJidList }) => {
        if (message?.buttonsMessage?.locationMessage?.jpegThumbnail) {
            const thumb = message.buttonsMessage.locationMessage.jpegThumbnail;
            try {
                let inputBuffer;
                ...
                if (inputBuffer && inputBuffer.length > 0) {
                    message.buttonsMessage.locationMessage.jpegThumbnail = await sharp(inputBuffer)
        ...
        if (message?.buttonsMessage?.headerType === 6 && message?.buttonsMessage?.locationMessage) {
            message.buttonsMessage.headerType = 6;
        }
        await processJpegThumbnails(message);
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            if (typeof content === 'object' && 'cards' in content && content.cards) {
                const c = content;
                const cards = await Promise.all(c.cards.map(async (card) => {
                    ...
                    if (card.image) {
                        const img = await prepareWAMessageMedia({ image: card.image }, { upload: waUploadToServer, logger, mediaCache: config.mediaCache });
                        cardBody.header.hasMediaAttachment = true;
                        cardBody.header.imageMessage = img.imageMessage;
                    }
                    if (card.video) {
                        const vid = await prepareWAMessageMedia({ video: card.video }, { upload: waUploadToServer, logger, mediaCache: config.mediaCache });
                        cardBody.header.hasMediaAttachment = true;
                        cardBody.header.videoMessage = vid.videoMessage;
                    }
                    return cardBody;
                }));
                ...
                const fullMsg = generateWAMessageFromContent(jid, {
                    interactiveMessage: {
                        header: { title: c.title || '', subtitle: c.subtitle || '', hasMediaAttachment: false },
                        body: { text: c.text || '' },
            ...
            if (typeof content === 'object' &&
                ...
                const albumHeader = generateWAMessageFromContent(jid, {
                    albumMessage: {
                        expectedImageCount: albums.filter(item => 'image' in item).length,
                        expectedVideoCount: albums.filter(item => 'video' in item).length
                    }
                }, opts);
                const albumId = albumHeader.key.id;
                await relayMessage(jid, albumHeader.message, {
                    messageId: albumId,
                    useCachedGroupMetadata: options.useCachedGroupMetadata,
                    additionalAttributes: options.additionalAttributes,
                    statusJidList: options.statusJidList,
                ...
                if (config.emitOwnEvents) {
                    process.nextTick(async () => {
                        await messageMutex.mutex(() => upsertMessage(albumHeader, 'append'));
                    });
                }
                const resultMessages = [albumHeader];
                for (const media of albums) {
                    await delay(options.delay || 500);
                    let mediaContent = {};
                    if ('image' in media) mediaContent = { image: media.image };
                    else if ('video' in media) mediaContent = { video: media.video };
                    else continue;
                    const mediaMsg = await generateWAMessage(jid, {
                        ...mediaContent,
                        ...media,
                        albumParentKey: { remoteJid: jid, fromMe: true, id: albumId }
                    }, { ...opts, messageId: generateMessageIDV2(sock.user?.id) });
                    mediaMsg.message.messageContextInfo = {
                        messageSecret: randomBytes(32),
                        messageAssociation: {
                            associationType: proto.MessageAssociation.AssociationType.MEDIA_ALBUM,
                            parentMessageKey: { remoteJid: jid, fromMe: true, id: albumId }
                        }
                    };
                    ...
                    resultMessages.push(mediaMsg);
                }
                return resultMessages;
            }
            else if (typeof content === 'object' && 'interactiveButtons' in content && content.interactiveButtons) {
                    ...
                    additionalNodes: [
                        {
                            tag: 'biz',
                            attrs: {
                                native_flow_name: buttons[0]?.name || 'payment_info'
                            }
                        }
                    ]
                });
            ...
            else if (typeof content === 'object' && 'event' in content && content.event) {
                ...
                const msg = generateWAMessageFromContent(jid, {
                    ...
                    eventMessage: {
                        ...
                        joinLink: ev.joinLink || '',
                        startTime: typeof ev.startTime === 'string' ? parseInt(ev.startTime) : (ev.startTime || Math.floor(Date.now() / 1000) + 86400),
                        endTime: typeof ev.endTime === 'string' ? parseInt(ev.endTime) : ev.endTime || undefined,
                        extraGuestsAllowed: ev.extraGuestsAllowed ?? true,
                        isScheduleCall: ev.isScheduleCall ?? false,
                        hasReminder: ev.hasReminder ?? true,
                        reminderOffsetSec: ev.reminderOffsetSec || 3600
                    }
                }, {
                    logger,
                    userJid,
            ...
            else if (typeof content === 'object' && 'extendedText' in content && content.extendedText) {
                const et = content.extendedText;
                const msg = generateWAMessageFromContent(jid, {
                    messageContextInfo: {
                        messageSecret: randomBytes(32)
                    },
                    extendedTextMessage: {
                        text: et.text || '',
                        matchedText: et.matchedText || undefined,
                        canonicalUrl: et.canonicalUrl || undefined,
                        description: et.description || undefined,
                        title: et.title || undefined,
                        font: et.font || undefined,
                        previewType: et.previewType || undefined,
                        jpegThumbnail: et.jpegThumbnail || undefined,
                        contextInfo: et.contextInfo || content.contextInfo || undefined,
                        doNotPlayInline: et.doNotPlayInline || false,
                        thumbnailDirectPath: et.thumbnailDirectPath || undefined,
                        thumbnailSha256: et.thumbnailSha256 || undefined,
                        thumbnailEncSha256: et.thumbnailEncSha256 || undefined,
                        mediaKey: et.mediaKey || undefined,
                        mediaKeyTimestamp: et.mediaKeyTimestamp || undefined,
                        thumbnailHeight: et.thumbnailHeight || undefined,
                        thumbnailWidth: et.thumbnailWidth || undefined,
                        inviteLinkGroupTypeV2: et.inviteLinkGroupTypeV2 || undefined
                    }
                }, {
                    logger,
                    userJid,
                ...
                const isInteractiveMsg = 'interactiveMessage' in content || 'interactive' in content || 'buttonsMessage' in content || 'listMessage' in content || 'templateMessage' in content || !!fullMsg.message?.interactiveMessage || !!fullMsg.message?.viewOnceMessage?.message?.interactiveMessage || !!fullMsg.message?.viewOnceMessageV2?.message?.interactiveMessage || !!fullMsg.message?.viewOnceMessageV2Extension?.message?.interactiveMessage || !!content.nativeFlowName || !!options.nativeFlowName || !!content.interactiveType || !!options.interactiveType;
                const isBotMsg = ('bot' in content && !!content.bot) || ('ai' in content && !!content.ai) || ('biz_bot' in content && !!content.biz_bot) || !!options.ai || !!options.bot || !!fullMsg.message?.botInvokeMessage;
                const additionalAttributes = {};
                const additionalNodes = [];
                if (Array.isArray(options.additionalNodes)) {
                    additionalNodes.push(...options.additionalNodes);
                }
                if (Array.isArray(content.additionalNodes)) {
                    additionalNodes.push(...content.additionalNodes);
                }
                // required for delete
                if (isDeleteMsg) {
                    ...
                    const msgStr = JSON.stringify(fullMsg.message || content);
                    let flowName = content.nativeFlowName || options.nativeFlowName || content.interactiveType || options.interactiveType;
                    if (!flowName) {
                        if (msgStr.includes('"name":"mixed"')) flowName = 'mixed';
                        else if (msgStr.includes('"name":"payment_key_info"')) flowName = 'payment_key_info';
                        else if (msgStr.includes('"name":"catalog_message"')) flowName = 'catalog_message';
                        else if (msgStr.includes('"name":"order_details"')) flowName = 'order_details';
                    }
                    if (flowName === 'mixed') {
                        additionalNodes.push({
                            tag: 'biz',
                    ...
                    else if (flowName === 'payment_key_info') {
                        additionalNodes.push({
                            tag: 'biz',
                            attrs: {},
                            content: [
                    ...
                    else if (flowName === 'catalog_message') {
                        additionalNodes.push({
                            tag: 'biz',
                            attrs: {
                    ...
                    else if (flowName === 'order_details') {
                        additionalNodes.push({
                            tag: 'biz',
                            attrs: {
                        ...
                        additionalNodes.push({
                            tag: 'biz',
                            attrs: flowName ? { native_flow_name: flowName } : {}
                        });
                    }
                }
                if (isBotMsg) {
