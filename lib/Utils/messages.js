...
const extractDisplayText = (button) => {
    ...
    return displayTextCandidates.find(value => typeof value === 'string' && value.length > 0) || '';
};
const normalizeButton = (button) => {
    var _a;
    if (button.name && typeof button.buttonParamsJson === 'string') {
        return { name: button.name, buttonParamsJson: button.buttonParamsJson };
    }
    if (button.name === 'cta_url') {
        return {
            name: 'cta_url',
            buttonParamsJson: JSON.stringify({
                display_text: button.displayText || button.display_text || button.text || '',
                url: button.url || '',
                merchant_url: button.merchant_url || button.merchantUrl || '',
            })
        };
    }
    if (button.name === 'cta_call') {
        return {
            ...
            buttonParamsJson: JSON.stringify({
                display_text: button.displayText || button.display_text || button.text || '',
                phone_number: button.phoneNumber || button.phone_number || '',
            })
        };
    }
    if (button.name === 'cta_copy') {
        return {
            ...
            buttonParamsJson: JSON.stringify({
                ...
                copy_code: button.copyCode || button.copy_code || '',
            })
        };
    }
    if (button.name === 'cta_reminder') {
        return {
            name: 'cta_reminder',
            buttonParamsJson: JSON.stringify({
                display_text: button.displayText || button.display_text || button.text || '',
    ...
    if (button.name === 'cta_cancel_reminder') {
        return {
    ...
    if (button.name === 'address_message') {
        return {
    ...
    if (button.name === 'send_location') {
        return {
            name: 'send_location',
            buttonParamsJson: JSON.stringify({})
        };
    }
    if (button.name === 'single_select' || button.name === 'multi_select') {
        return {
            name: button.name,
            buttonParamsJson: JSON.stringify({
                title: button.title || '',
    ...
    const displayText = extractDisplayText(button);
...
const normalizeQuickReplyButton = normalizeButton;
...
const generateWAMessageContent = async (message, options) => {
    ...
    if ('buttons' in message && !!message.buttons) {
        const hasMedia = !!(m.imageMessage || m.videoMessage || m.documentMessage || m.locationMessage);
        const interactiveMessage = {
            nativeFlowMessage: Types_1.WAProto.Message.InteractiveMessage.NativeFlowMessage.fromObject({
                buttons: message.buttons.map(normalizeButton),
            })
        };
        if ('text' in message) {
            ...
            interactiveMessage.header = {
                ...
                hasMediaAttachment: hasMedia || Boolean(message.hasMediaAttachment),
            };
            Object.assign(interactiveMessage.header, m);
        }
        if (hasMedia && !interactiveMessage.header) {
            interactiveMessage.header = {
                title: message.title || '',
                subtitle: message.subtitle,
            ...
            Object.assign(interactiveMessage.header, m);
            if (!interactiveMessage.body) {
                interactiveMessage.body = { text: message.caption || message.text || '' };
            }
        }
